...
class AuthenticationRepository {
  ...
  Stream<core.AuthCredential> get credential {
    return _firebaseAuth.authStateChanges().map((firebaseUser) {
      if (firebaseUser == null) {
        return core.AuthCredential.empty;
      } else {
        return firebaseUser.toCredential;
      }
    });
  }
  ...
  /// Creates an account on the server using Firebase ID token
  Future<void> _createAccountOnServer({
    ...
    try {
      ...
      final response = await _dio.post(
        '$_apiBaseUrl$_createAccountEndpoint',
        options: Options(
          headers: {
      ...
      if (response.statusCode != 200) {
        throw Exception(
          'Failed to create account on server: ${response.statusCode}',
        );
      }
    } catch (e) {
      throw Exception('Failed to create account on server: $e');
    }
  }
  ...
  Future<void> logInWithGoogle() async {
    try {
      ...
      // Get the ID token for API authentication and create account on server
      final user = userCredential.user;
      if (user != null && user.email != null) {
        final idToken = await user.getIdToken();

        if (idToken != null) {
          // Create account on the server (will be get_or_create, so safe for existing users)
          await _createAccountOnServer(
            idToken: idToken,
            email: user.email!,
            uid: user.uid,

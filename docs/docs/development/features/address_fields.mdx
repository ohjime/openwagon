...
- Cotton component template: `src/server/lib/dispatch/templates/cotton/address_route.html`
...
Cotton resolves `<c-address-route />` to `dispatch/templates/cotton/address_route.html`. The optional `api-key` attribute lets you pass a Google Maps API key from Django.
